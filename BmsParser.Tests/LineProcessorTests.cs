...
namespace BmsParser.Tests
{
    ...
    {
        [DataTestMethod]
        [DataRow("#PLAYER 2", 2, nameof(BmsModel.Player))]
        ...
        [DataTestMethod]
        [DataRow("#RANK 10", "#RANK")]
        [DataRow("#PLAYER 123", "#PLAYER")]
        public void LoadNumberOutOfRange(string line, string name)
        {
            var model = new BmsModel(Mode.Beat7K);
            var logs = new List<DecodeLog>();
            ...
            Assert.IsTrue(logs.Count != 0);
            Assert.AreEqual(State.Warning, logs[0].State);
            Assert.AreEqual($"{name}に規定外の数字が定義されています : {line}", logs[0].Message);
        }
        ...
        public void LoadWavSequence(string line, int seq, string value)
        {
            ...
            var processor = new LineProcessor();
            processor.Process(model, line, logs);
            Assert.AreEqual(value, processor.WavList[processor.WavMap[seq]]);
        }
        ...
        public void LoadBmpSequence(string line, int seq, string value)
        {
            ...
            var processor = new LineProcessor();
            processor.Process(model, line, logs);
            Assert.AreEqual(value, processor.BgaList[processor.BgaMap[seq]]);
        }
